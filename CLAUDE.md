...
- API calls go straight to `http://localhost:8000` (the `API` const) rather than through Next's own API routes — `next.config.js`'s `rewrites()` config is currently unused dead config, not the actual request path.
